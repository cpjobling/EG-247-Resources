...
ython.org/github/cpjobling/EG-247-Resources/blob/master/week5/fourier1.ipynb)
...
Analsysis and Trigonometric Fourier Series.
...
* An *harmonic* is a frequency whose value is an integer multiple of some
...
* A periodic function with period $T$, has *half-wave symmetry* if $f(t) = -f(t
+ T/2)$
...
(4*A*sin(t))/pi + (4*A*sin(3*t))/(3*pi) + (4*A*sin(5*t))/(5*pi) +
(4*A*sin(7*t))/(7*pi) + (4*A*sin(9*t))/(9*pi) + (4*A*sin(11*t))/(11*pi)
```
...
* The waveform has half-wave symmetry, so only odd indexed coeeficents will be
present.
...
(4*A*cos(t))/pi - (4*A*cos(3*t))/(3*pi) + (4*A*cos(5*t))/(5*pi) -
(4*A*cos(7*t))/(7*pi) + (4*A*cos(9*t))/(9*pi) - (4*A*cos(11*t))/(11*pi)
```
